...
@RestController
@RequestMapping("/department/infomation")
public class DepartmentInfomationController extends BaseController{

    @PostMapping("/options/{userId}/{teachId}")
    @PreAuthorize("hasAuthority('department:infomation:options')")
    ...
    @PreAuthorize("hasAuthority('department:infomation:list')")
    ...
    @PreAuthorize("hasAuthority('department:infomation:batch')")
    @GetMapping("/batch")
    public Result batch() {
